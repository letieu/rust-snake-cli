...
use crossterm::style::{ResetColor, Print, SetForegroundColor, Color};
...
impl Game {
    pub fn new(stdout: Stdout, width: u16, height: u16) -> Self {
        ...
        Self {
            ...
            height,
            food: None,
            snake: Snake::new(Point::new(width / 2, height / 2), 3, Direction::Right),
            speed: 10,
            score: 0,
        }
    }
        ...
        while !done {
            ...
            while now.elapsed() < interval {
                if let Some(command) = self.get_command(interval - now.elapsed()) {
                    match command {
                        ...
                        Command::Turn(new_direction) => {
                            if direction.opposite() != new_direction && direction != new_direction {
            ...
            if self.has_collided_with_wall() || self.has_bitten_itself() {
                ...
                if let Some(food_point) = self.food {
                    if self.snake.get_head() == &food_point {
                        self.snake.digest();
                        self.place_food();
                        self.score += 1;
    ...
    fn get_command(&self, wait_for: Duration) -> Option<Command> {
        ...
        match key_event.code {
            ...
            crossterm::event::KeyCode::Up => Some(Command::Turn(Direction::Up)),
            crossterm::event::KeyCode::Down => Some(Command::Turn(Direction::Down)),
            crossterm::event::KeyCode::Left => Some(Command::Turn(Direction::Left)),
            crossterm::event::KeyCode::Right => Some(Command::Turn(Direction::Right)),
            _ => None,
        }
    }
    ...
    fn has_collided_with_wall(&self) -> bool {
        let head_point = self.snake.get_head();
    ...
    fn has_bitten_itself(&self) -> bool {
        let next_head_point = self.snake.get_head().transform(*self.snake.direction(), 1);
